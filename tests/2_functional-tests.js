...
suite('Functional Tests', function() {
  ...
  suite('POST Tests', function() {
    test("Create an issue with every field: POST request to /api/issues/{project}", done => {
      ...
      .request(server)
      .keepOpen()
      .post(`/api/issues/chant?issue_title=test 1&issue_text=test&created_by=chant&assigned_to=chant&status_text=test`)
      .end(function (err, res) {
          assert.equal(err, null);
          assert.equal(res.status, 200);

          let { issue_title, issue_text, status_text, created_by, assigned_to, ...other } = JSON.parse(res.text);
    ...
    test("Create an issue with only required fields: POST request to /api/issues/{project}", done => {
      ...
      .request(server)
      .keepOpen()
      .post(`/api/issues/chant?issue_title=test 2&issue_text=test&created_by=chant`)
      .end(function (err, res) {
          assert.equal(err, null);
          assert.equal(res.status, 200);
  ...
  suite('PUT Tests', function() {
    ...
    test("Update multiple fields on an issue: PUT request to /api/issues/{project}", done => {
      ...
      .end(function (err, res) {
        ...
        .request(server)
        .keepOpen()
        .put("/api/issues/chant?_id=" + _id + "&assigned_to=chant&issue_text=put test")
        .end(function (err, res) {
            assert.equal(err, null);
            assert.equal(res.status, 200);
            assert.equal(res.text, `{"result":"successfully updated","_id":"${_id}"}`);
